...
def main() -> int:
    ...
    ap.add_argument("--cloth", type=Path, required=True, help="VITON-HD ürün fotoğrafları dizini")
    ...
    for i, img_path in enumerate(images):
        ...
        if not cloth_path.exists():
            failed.append(f"{img_path.stem}: cloth yok")
            continue
        try:
            ...
            bundle = build_conditioning(
                ...
                person_prep=pp,
                appearance_ref_image=np.asarray(Image.open(cloth_path).convert("RGB")),
            )
